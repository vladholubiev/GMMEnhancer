(function() {
	...
	$(document).ready(function() {
		...
		function insert(n) {
			$('.block.read.hide-for-lightweight-ui').eq(0).click();
			if ($('.block.edit input.jfk-textinput').length === 1) {
				count = 0;
			}
			$('.block.edit input.jfk-textinput').eq(count).val(n);
			$('.block.read.hide-for-lightweight-ui').eq(0).click();
			$('#gw-saveedit').click();
		...
		function main() {
			count = 1, change = '<div id="change" class="level goog-inline-block jfk-button jfk-button-standard jfk-button-collapse-left jfk-button-clear-outline">' + changeText + '</div>', template = ['<div id="level', '" class="level goog-inline-block jfk-button jfk-button-standard jfk-button-narrow jfk-button-collapse-left jfk-button-clear-outline">', '</div>'];
