...
(function() {
	...
	$('#kd-browse-toolbar-button[style*="max-width: 250px;"]').click(function() {
		//If > 0 blocks present - remove last
		if ($('textarea#shortList').length > 0) {
			$('#panel_7_1').remove();
		}
		$('div.item').eq(0).before('<div class="item" id="panel_7_1">' +
            '<div class="result gw-feature-summary"><div class="icon">' +
            '<img src="/mapmaker/mapfiles/note-icon-k.png"></div>' +
            '<div class="header"><a class="title">' + chrome.i18n.getMessage("sl_block_name") + '</a></div>' +
            '<div class="stream"><div style="display:">' +
            '<div class="action edit"><div><span class="hc" uid="211700403834496622904">' +
            '<textarea id="shortList" cols="65" rows="15" readonly="" autofocus="" onclick="this.focus();this.select()">&#14;' +
            '</textarea></span>' +
            '<div class="content-end"></div></div></div>' +
            '<div class="action"></div></div></div></div></div>');
		main();
	});
})();