...
$(document).ready(function () {
    ...
    $('#newTemplate').click(function () {
        chrome.storage.local.get('disallowDrawing', function(disallowDrawing) {
            console.log(disallowDrawing);
            if (disallowDrawing.disallowDrawing != true) {
                var name = prompt("Enter new template name");
                chrome.runtime.sendMessage({
                    greeting: "startRecordPath",
                    templateName: name
                });
                chrome.tabs.executeScript({
                    file: 'js/lib/svg.min.js'
                }, function () {
                    chrome.tabs.executeScript({
                        file: 'js/gmm/drawingTemplates.js'
                    }, function () {
    ...
    $('#stopRecordPath').click(function () {
        chrome.runtime.sendMessage({
            greeting: "stopRecordPath"
        });
    });
    $('#saveRecordPath').click(function () {
        chrome.runtime.sendMessage({
            greeting: "saveRecordPath"
        });
    });
});